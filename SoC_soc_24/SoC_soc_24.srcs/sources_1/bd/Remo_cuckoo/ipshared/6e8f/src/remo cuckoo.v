...
module cuckoo_uart(
    ...
    input rx,               // UART 수신 핀
    output tx,              // UART 송신 핀
    output reg [6:0] btn_value, // 값을 내보내기
    output reg set_watch_value
);
    ...
    always @(posedge clk or posedge reset_p) begin
        if (reset_p) begin
            ...
            send <= 1'b0;
            state <= IDLE;
            btn_value <= 7'b000_0000; // btn_value 초기화
            set_watch_value <= 1'b0;
        end
        else begin
            case (state)
                IDLE: begin
                    if (received_flag) begin
                        ...
                        case (received_data)
                            8'h31: set_watch_value <= 1'b1; // '1'에 해당하는 ASCII 코드
                            8'h32: btn_value <= 7'b000_0010; // '2'에 해당하는 ASCII 코드
                            8'h33: btn_value <= 7'b000_0100; // '3'에 해당하는 ASCII 코드
                            8'h34: btn_value <= 7'b000_1000; // '4'에 해당하는 ASCII 코드
                            8'h35: btn_value <= 7'b001_0000; // '5'에 해당하는 ASCII 코드
                            8'h36: btn_value <= 7'b010_0000; // '6'에 해당하는 ASCII 코드
                            8'h37: btn_value <= 7'b100_0000; // '7'에 해당하는 ASCII 코드
                            default: btn_value <= 7'b000_0000; // 다른 값일 경우 초기화
                        endcase
