...
//            T_flip_flop_p               t_mode( .clk(clk), .reset_p(reset_p) , .t(btn_mode), .q(set_watch));
            ...
            clock_div_60    min_clock   (.clk(clk), .reset_p(reset_p), .up_clk_source(inc_sec), .down_clk_source(dec_sec), .clk_div_60_nedge(clk_min));   
            clock_div_60    hour_clock  (.clk(clk), .reset_p(reset_p), .up_clk_source(inc_min), .down_clk_source(dec_min), .clk_div_60_nedge(clk_hour));   
...
//                                                                                         .load_hour(watch_hour),.hour(set_hour));
            ...
            wire inc_sec, inc_min, dec_sec, dec_min;    //초증가, 분증가를 저장할 변수
            assign inc_sec = set_watch ? btn_sec_up : clk_sec; // set_watch 가 1일때  //mux
            assign dec_sec = set_watch ? btn_sec_down : 0; // set_watch 가 1일때  //mux
            assign inc_min = set_watch ? btn_min_up: clk_min; //분 증가 //mux
            assign dec_min = set_watch ? btn_min_down: 0; //분 증가 //mux
